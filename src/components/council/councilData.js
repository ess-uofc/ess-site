import president from "../../assets/council/2026-2027/President.png";
import vpAcademic from "../../assets/council/2026-2027/VP Academic.png";
import vpCommunications from "../../assets/council/2026-2027/VP Communications.png";
import vpCorporateRelations from "../../assets/council/2026-2027/VP Corporate Relations.png";
import vpExternal from "../../assets/council/2026-2027/VP External.png";
import vpFinance from "../../assets/council/2026-2027/VP Finance.png";
import vpInternal from "../../assets/council/2026-2027/VP Internal.png";
import vpOperations from "../../assets/council/2026-2027/VP Operations.png";
import vpStudentLife from "../../assets/council/2026-2027/VP Student Life.png";

import adminCommissioner from "../../assets/council/2026-2027/Admin Commissioner.png";
import crCommissioner from "../../assets/council/2026-2027/CR Commissioner.png";
import compsCommissioner from "../../assets/council/2026-2027/Comps Commissioner.png";
import compsCommissioner2 from "../../assets/council/2026-2027/Comps Commissioner (2).png";
import corporateDirector from "../../assets/council/2026-2027/Corporate Director.png";
import designCommissioner from "../../assets/council/2026-2027/Design Commissioner.png";
import diversityCommissioner from "../../assets/council/2026-2027/Diversity Commissioner.png";
import dinnerDirector from "../../assets/council/2026-2027/Dinner Director.png";
import financeCommissioner from "../../assets/council/2026-2027/Finance Commissioner.png";
import gradDirector from "../../assets/council/2026-2027/Grad Director.png";
import proDevCommissioner from "../../assets/council/2026-2027/Pro Dev Comm.png";
import publicationsCommissioner from "../../assets/council/2026-2027/Publications Commissioner.png";
import socialCommissioner from "../../assets/council/2026-2027/Social Commissioner.png";
import socialCommissioner2 from "../../assets/council/2026-2027/Social Commissioner (2).png";
import socialCommissioner3 from "../../assets/council/2026-2027/Social Commissioner (3).png";
import socialMediaDirector from "../../assets/council/2026-2027/Social Media Director.png";
import sponsorshipCommissioner from "../../assets/council/2026-2027/Sponsorship Commissioner.png";
import technologyCommissioner from "../../assets/council/2026-2027/Technology Commissioner.png";
import websiteCommissioner from "../../assets/council/2026-2027/Website Commissioner.png";


const councilMembers = [

    {
        id: 1,
        name: "Daniel Yang",
        role: "President",
        year: "3rd Year",
        major: "Civil",
        description: `Current song on repeat "Next Levels by King Geedorah."

A life goal of mine is…
To help promote mental health and create more support for students and young adults.

I get way too excited about…
Volleyball! I've played for most of my life and I hope to continue doing so. I’m always down to play!`,
        image: president,
        email: "president@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/daniel-yang587"
    },

    {
        id: 2,
        name: "Luke Zenha",
        role: "VP Academic",
        year: "3rd Year",
        major: "Electrical",
        description: `A fun fact about myself is I'm double joined in my thumb

My favourite engineering memory is spending BSD last year working on my 300 project (it’s only fun in retrospect..)

I get way too ecited about cheese and crackers`,
        image: vpAcademic,
        email: "academic@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/luke-zenha-054a2033a/"
    },

    {
        id: 3,
        name: "Cathy Xie",
        role: "VP Communications",
        year: "3rd Year",
        major: "Mechanical & Aerospace Minor",
        description: `I get way too excited about rock climbing

A fun fact about myself is I have my pilots license

Current song on repeat "King for a Day" by Pierce the Veil`,
        image: vpCommunications,
        email: "communications@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/cathy-xie-535aa2326?utm_source=share_via&utm_content=profile&utm_medium=member_ios"
    },

    {
        id: 4,
        name: "Antonio Alapizco-Bonardel",
        role: "VP Corporate Relations",
        year: "2nd Year",
        major: "Mechanical",
        description: `A life goal of mine is to hike to Machu Picchu

The most spontaneous thing I've done is go across Europe for a summer

I get way too excited about going to museums to look at old paintings`,
        image: vpCorporateRelations,
        email: "corporaterelations@essucalgary.com",
        linkedin: "http://www.linkedin.com/in/antonio-alapizco-bonardel-7b786b37a"
    },

    {
        id: 5,
        name: "Elyse Wookey",
        role: "VP External",
        year: "3rd Year",
        major: "Chemical/Business",
        description: `A life long goal of mine is to work overseas

My mantra is dance like no one's watching

I get way too excited about spikeball!`,
        image: vpExternal,
        email: "external@essucalgary.com",
        linkedin: ""
    },

    {
        id: 6,
        name: "Zach Grappolini",
        role: "VP Finance",
        year: "3nd Year",
        major: "Mechanical",
        description: `I get way too excited about Excel

My mantra is Excel Excel Excel

Two truths and a lie I love excel, I dislike Google Sheets, I hate pivot tables`,
        image: vpFinance,
        email: "finance@essucalgary.com",
        linkedin: ""
    },

    {
        id: 7,
        name: "Fatima Zafar",
        role: "VP Internal",
        year: "3rd Year",
        major: "Electrical",
        description: `I get way to excited about concerts

A current song on repaet "Thinking Bout You" by Ariana Grande

My Mantra is it's not over till it's over`,
        image: vpInternal,
        email: "internal@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/fatima-zafar-ab41782a6"
    },

    {
        id: 8,
        name: "Taylor Kindleman",
        role: "VP Operations",
        year: "5th Year",
        major: "Mechanical",
        description: `My favourite engineering memory is surviving thermo 2. I don't talk about it, but I did come out the other side with a deep personal grudge against the Carnot Cycle.

Two truths and a lie I've taken apart an engine and successfully put it back together. I once spent more time designing a 3D printed part than the thing it was fixing. I've never Googled something mid-conversation to win an argument.

Current sonf on repeat "Bitin' List" by Tyler Childers`,
        image: vpOperations,
        email: "operations@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/taylor-k/"
    },

    {
        id: 9,
        name: "Cameron Bews (the one one the right)",
        role: "VP Studen Life",
        year: "3rd Year",
        major: "Mechanical",
        description: `A fun fact about myself I love ranch

I get way too excited about Ranch dressing

Current song on repeat "Shine a Little Light" by The Black Keys`,
        image: vpStudentLife,
        email: "studentlife@essucalgary.com",
        linkedin: ""
    },

    {
        id: 10,
        name: "Logan Kim",
        role: "Administration Commissioner",
        year: "3rd Year",
        major: "Mechanical",
        description: `A life goal of mine is to help my parents retire

My mantra is go with the flow

Current song on repeat "If 6 was 9" by Jimi Hendrix`,
        image: adminCommissioner,
        email: "admin.comm@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/minsung-kim-meng/"
    },

    {
        id: 11,
        name: "Rayyan Hussain",
        role: "Community Relations Commissioner",
        year: "2nd Year",
        major: "Mechanical",
        description: `I get way too excited about F1

A life goal of mine is to go on a world tour and explore new cultures

My favorite engineering memory is fresh air after exams season`,
        image: crCommissioner,
        email: "community@essucalgary.com",
        linkedin: "http://www.linkedin.com/in/rayyan-hus"
    },

    {
        id: 12,
        name: "Erin Kim",
        role: "Competitions Commissioner",
        year: "4th Year",
        major: "Software",
        description: `I get way too excited about Baldur's Gate 3

A life goal of mine is to go solo camping one day

Current song on repeat "TBH" by PARTYNEXTDOOR`,
        image: compsCommissioner,
        email: "competitions@essucalgary.com",
        linkedin: ""
    },

    {
        id: 13,
        name: "Kambi Obodo",
        role: "Competitions Commissioner",
        year: "3rd Year",
        major: "Geomatics",
        description: `Two truths and a lie I have 4 pet snakes, I did ballet when I was younger and I have never broken a bone.

I get way too excited about the movie Encanto

My mantra is Hakuna Matata,`,
        image: compsCommissioner2,
        email: "competitions@essucalgary.com",
        linkedin: ""
    },

    {
        id: 14,
        name: "Shanaya Kailey",
        role: "Corporate Director Commissioner",
        year: "2nd Year",
        major: "Electrical",
        description: `I get way too excited about a yummy lunch or sweet treat!

A life goal of mine is to study abroad

Current song on repeat "Ring of Past by Men I Trust"`,
        image: corporateDirector,
        email: "corporatedirector@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/shanaya-kailey-562978389/"
    },

    {
        id: 15,
        name: "Noor Ali",
        role: "Design Commissioner",
        year: "3rd Year",
        major: "Software",
        description: `Current song on repeat "4Me 4Me" by Malcolm Todd

A fun fact about myself is I can play 5 musical instruments

A life goal of mine is to find a way for my cat to live longer than me`,
        image: designCommissioner,
        email: "artistic@essucalgary.com",
        linkedin: ""
    },

    {
        id: 16,
        name: "Savaarna Mitra",
        role: "Diversity Commissioner",
        year: "3rd Year",
        major: "Electrical",
        description: `Current song on repeat "Root of all evil" by Daniel Caesar

A life goal of mine is to learn how to integrate

I get way too excited about watching movies at the theatres`,
        image: diversityCommissioner,
        email: "diversity@essucalgary.com",
        linkedin: ""
    },

    {
        id: 17,
        name: "Jade Cosmilla",
        role: "Engineering Dinner Co-Director",
        year: "3rd Year",
        major: "Risk Management & Polisci",
        description: `A fun fact about myself is I can sleep anywhere

My mantra is ff you did what you could, you did your best

I get way too excited about Antreaters!!`,
        image: dinnerDirector,
        email: "dinner.director1@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/jade-cosmilla-118964332"
    },

    {
        id: 18,
        name: "Kate Schuster",
        role: "Finance Commissioner",
        year: "3rd Year",
        major: "Biomedical",
        description: `A fun fact about myself is I am a medical first responder!

Current song on repeat "Opalite" by Taylor Swift

My favorite engineering memory is going to CELC last year!`,
        image: financeCommissioner,
        email: "finance.comm@essucalgary.com",
        linkedin: ""
    },

    {
        id: 19,
        name: "Allison Hunchak",
        role: "Graduation Director",
        year: "2nd Year",
        major: "Civil",
        description: `Current song on repeat "End of August" by Noah Kahan

A life goal of mine is to backpack through Europe for a summer

I get way too excited about concerts and live music`,
        image: gradDirector,
        email: "grad.director@essucalgary.com",
        linkedin: "https://ca.linkedin.com/in/allison-h-73809039b"
    },

    {
        id: 20,
        name: "Phuc (Bernard) Le",
        role: "Professional Development Commissioner",
        year: "2nd Year",
        major: "Chemical",
        description: `A fun fact about myself is I can eat 5 bowls of rice in one sitting

I get way too excited about free food

A life goal of mine is to do a cross country road trip from California to North Carolina`,
        image: proDevCommissioner,
        email: "pdevelopment@essucalgary.com",
        linkedin: ""
    },

    {
        id: 21,
        name: "Manal Chaudhry",
        role: "Publications Commissioner",
        year: "Internship",
        major: "Mech/Elec dual major",
        description: `Current song on repeat "Light of the Seven" by Ramin Djawadi (GOT fans hmu)

I get way too excited about trying new hobbies

A life goal of mine is to learn piano and play in a jazz band`,
        image: publicationsCommissioner,
        email: "publications@essucalgary.com",
        linkedin: ""
    },

    {
        id: 22,
        name: "Angela Bohora",
        role: "Social Commissioner",
        year: "2nd Year",
        major: "Civil",
        description: `A fun fact about myself is I collect a lot of things but sometimes I think I’m just a hoarder

A life goal of mine is to I'd love to backpack Europe and buy myself a pair of izzys, don’t know which one I want more

Current song on repeat "Chosen" by Blood Orange & Caroline Polacheck`,
        image: socialCommissioner,
        email: "social.comm1@essucalgary.com",
        linkedin: ""
    },

    {
        id: 23,
        name: "Ryman Almond",
        role: "Social Commissioner",
        year: "2nd Year",
        major: "Chemical",
        description: `I get way too excited about hockey

A fun fact about myself is one of my favorite hobbies is wind surfing!

Current song on repeat "Say my Name" by Hozier`,
        image: socialCommissioner2,
        email: "social.comm3@essucalgary.com",
        linkedin: ""
    },

    {
        id: 24,
        name: "Chiedu Ngene",
        role: "Social Commissioner",
        year: "3rd Year",
        major: "Electrical",
        description: `My mantra is actions have consequences

I get way too excited about new movies and music

Current song on repeat "Back Outside" by BNXN`,
        image: socialCommissioner3,
        email: "social.comm2@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/chiedu-ngene/"
    },

    {
        id: 25,
        name: "Eileen Cai",
        role: "Social Media Director",
        year: "2nd Year",
        major: "Biomedical",
        description: `Current song on repeat "UP!" by GASPXR

I get way too excited about knowing I can get 9 hours of sleep (I still won't do it though)

A life goal of mine is to quit playing League of Legends`,
        image: socialMediaDirector,
        email: "social.media.director@essucalgary.com",
        linkedin: ""
    },

    {
        id: 26,
        name: "Bisola Jenyo",
        role: "Sponsorship Commissioner",
        year: "2nd Year",
        major: "Mechanical",
        description: `I get way too excited about C-dramas

A fun fact about myself is I love dancing

Current sogn on repeat "Window Seat" by Erykah Badu`,
        image: sponsorshipCommissioner,
        email: "sponsorship@essucalgary.com",
        linkedin: ""
    },

    {
        id: 27,
        name: "Rabi Khan",
        role: "Technology Commissioner",
        year: "3rd Year",
        major: "Software",
        description: `My cry-in-the-car song is "Seigfried" by Frank Ocean.

My life goal is to make enough money to never work again and just work on fun projects at home.

This year I really want to hit Challenger on League of Legends`,
        image: technologyCommissioner,
        email: "technology@essucalgary.com",
        linkedin: "https://www.linkedin.com/in/rabimkhan/"
    },

    {
        id: 28,
        name: "Artur Mararash",
        role: "Website Commissioner",
        year: "2nd Year",
        major: "Mechanical",
        description: `I get way to excited about cars cars cars

A fun fact about myself is I can bench press 260lbs (158 bw)

A life long goal of mine is to survive in this economy`,
        image: websiteCommissioner,
        email: "website@essucalgary.com",
        linkedin: "www.linkedin.com/in/artur-mararash-86a4a937b/"
    },

];

export default councilMembers;